...
public class PrintUtil {
    ...
    public static void printTest(BluetoothSocket bluetoothSocket, String name,String idNumber,String
                                 time,String carNumber,String place,String type,String law,String punishmentLaw,
                                 String puniment,String punishemenName) {

        try {
            PrintUtil pUtil = new PrintUtil(bluetoothSocket.getOutputStream(), "GBK");
            // 店铺名 居中 放大
            pUtil.printAlignment(1);
            pUtil.printLargeText("处罚决定书");
            pUtil.printLine();
            pUtil.printAlignment(0);
            pUtil.printLine();

            pUtil.printTwoColumn("被处罚人:", name);
            pUtil.printLine();

            pUtil.printTwoColumn("身份证号:", idNumber);
            pUtil.printLine();

            pUtil.printTwoColumn("车辆号牌:", carNumber);

            pUtil.printLine(2);
            //违法行为说明
            pUtil.printText("  该被处罚人于"+time+"，在"+place+"处，实施非机动车"+type+"违法行为，违反了"+
                    law+"的规定");

            pUtil.printLine(2);
            //惩罚说明
            pUtil.printText("  根据"+punishmentLaw+"规定，决定给予：");
            pUtil.printLine();
            pUtil.printText("  "+puniment+"处罚");
            pUtil.printLine(2);
            pUtil.printText("  如不服本决定，可以在收到本决定书之日起60日内向人民政府或公安局申请行政复议；" +
                    "或许在6个月内向人民法院提起行政诉讼");


            pUtil.printLine(2);

            pUtil.printTwoColumn("处罚地点:", place);
            pUtil.printLine();

            pUtil.printTwoColumn("处罚人:", punishemenName);
            pUtil.printLine();

            pUtil.printTwoColumn("处罚日期:", DateUtil.formatDate(new Date()));
